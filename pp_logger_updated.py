...
from autoware_control_msgs.msg import Control  # same output type
...
class PPLogger(Node):
    def __init__(self):
        ...
        # default output path for PP
        self.declare_parameter("dump_path", "/tmp/pp_log.jsonl")
        self.declare_parameter("head_k", 10)
        self.declare_parameter("rate_hz", 33.333333)  # dt ≈ 0.03s
        self.declare_parameter("algo", "pp")  # tag
        ...
        self.PP_OUTPUT: List[Dict[str, Any]] = []

        qos10 = QoSProfile(depth=10)

        # control_cmd publisher is RELIABLE + TRANSIENT_LOCAL + KEEP_LAST(1)
        ctrl_qos = QoSProfile(
        ...
        self.get_logger().info("=== Pure Pursuit Logger Started ===")
