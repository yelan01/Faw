...
namespace
{
inline double to_ms(const std::chrono::steady_clock::duration & d)
{
  return std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(d).count();
}

// Latest MPC optimization timing measured inside MPC::executeOptimization()
// (build QP matrices/bounds + solve). Values are stored in ms.
std::atomic<double> g_qp_build_ms{0.0};
std::atomic<double> g_qp_solve_ms{0.0};
std::atomic<double> g_qp_total_ms{0.0};
}  // namespace
  ...
  const Odometry & current_kinematics) const
{
  ...
  // [21-23] measured in MPC::executeOptimization() (QP build/solve/total, ms)
  append_diag(g_qp_build_ms.load(std::memory_order_relaxed));  // [21] qp_build_ms
  append_diag(g_qp_solve_ms.load(std::memory_order_relaxed));  // [22] qp_solve_ms
  append_diag(g_qp_total_ms.load(std::memory_order_relaxed));  // [23] qp_total_ms
  ...
  const double current_velocity)
{
  ...
  // Measure QP build + solve time inside executeOptimization().
  ...
  const bool solve_result = m_qpsolver_ptr->solve(H, f.transpose(), A, lb, ub, lbA, ubA, Uex);
  const auto t_solve_end = std::chrono::steady_clock::now();
  if (!solve_result) {
    // keep timing values as-is on failure
    return {ResultWithReason{false, "qp solver error"}, {}};
  }

  const double qp_build_ms = to_ms(t_solve_start - t_total_start);
  const double qp_solve_ms = to_ms(t_solve_end - t_solve_start);
  const double qp_total_ms = to_ms(t_solve_end - t_total_start);

  g_qp_build_ms.store(qp_build_ms, std::memory_order_relaxed);
  g_qp_solve_ms.store(qp_solve_ms, std::memory_order_relaxed);
  g_qp_total_ms.store(qp_total_ms, std::memory_order_relaxed);

  // Keep the old debug log (solver-only) but now using steady clock (ms).
  RCLCPP_DEBUG(m_logger, "qp solver calculation time = %.3f [ms]", qp_solve_ms);
